...
// ROM + attack/decay envelope), then a real BRAM-based reverb (reverb.sv:
// 3 parallel comb filters + 1 allpass, Freeverb-style), driving an 8-bit
// PWM DAC.
//
// Chime/bell character comes from:
...
//      than a piano-style table.
//   2) A long, slow envelope decay (~2.5s time constant) that keeps
//      oscillating at the last-struck pitch through the trailing rest
//      step, so each note rings out into the gap instead of being cut off.
//   3) A true time-domain reverb (see reverb.sv) applied to the dry
//      wavetable output before the PWM DAC.
//
