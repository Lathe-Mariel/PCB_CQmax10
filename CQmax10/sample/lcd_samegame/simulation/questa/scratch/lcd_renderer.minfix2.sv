...
module lcd_renderer #(
    ...
    // The product needs 17 bits (319*410 = 130790).  Evaluating the multiply
    // inside a 13-bit context truncates it and returns cx = 0 for every
    // pix_x >= 20, which puts the whole board in column 0.
    logic [17:0] mx, my;
    always_comb begin
        mx = pix_x * 9'd410;
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            // logo_ram's registered read already presents THIS request's word
            // in rom_data, so it pairs with empty1 (same request) directly.
            pix_color <= empty1 ? BG_COLOR : rom_data;
            pix_valid <= req1;
        end
    end
